...
def attack_and_evaluate(attack_type, original_image):
    if attack_type == 'no_attack':
        ...
        attacked_image_path = os.path.join('LSB/image/attack_result', 'no_attack.jpg')
    ...
    attacked_image_path = os.path.join('LSB/image/Attack_result', attacked_image_filename)
...
def main():
    # 載入原始圖像(修改過後)
    original_image = cv2.imread("LSB/image/Marked.jpg", 0)
    ...
    plt.savefig(os.path.join('LSB/result/Marked_Attack', 'all_attack_results.jpg'))
    plt.show()

    # Write results to a text file
    with open('LSB/result/Marked_Attack/attack_results.txt', 'w') as f:
        f.write('Attack Results for LSB\n\n')
